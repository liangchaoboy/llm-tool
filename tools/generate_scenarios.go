...
// TestScenario represents a single test case for our scenario
...
// calculateTokenCount estimates the number of tokens in text
...
func expandToTargetTokens(text string, targetTokens int) string {
	currentTokens := calculateTokenCount(text)
	...
	for calculateTokenCount(result) < targetTokens {
		if len(templates) > 0 {
			...
			if calculateTokenCount(newResult) <= targetTokens {
				result = newResult
			...
			if calculateTokenCount(result) >= targetTokens {
				break
			}
		}
	}
...
// generateScenarios creates test scenarios with realistic prompts
func generateScenarios(count int) []TestScenario {
	...
	for i := 0; i < count; i++ {
		...
		if isThinking {
			...
			scenario.Type = "thinking"
			scenario.InputTokens = 6000
			scenario.OutputTokens = 1000
			scenario.MaxTokens = 1000
			scenario.Temperature = 0.7
			basicPrompt := generateRealisticThinkingPrompt()
			scenario.Prompt = expandToTargetTokens(basicPrompt, 6000)
		} else {
			// Non-thinking case: 5500 input tokens, 500 output tokens
			scenario.Type = "non_thinking"
			scenario.InputTokens = 5500
			scenario.OutputTokens = 500
			scenario.MaxTokens = 500
			scenario.Temperature = 0.3
			basicPrompt := generateRealisticNonThinkingPrompt()
			scenario.Prompt = expandToTargetTokens(basicPrompt, 5500)
		}
...
func main() {
	fmt.Println("Generating realistic test scenarios...")

	// Generate test scenarios
	scenarios := generateScenarios(50000) // Generate full set of 50,000 scenarios
	...
	fmt.Printf("Successfully generated %d realistic test scenarios, saved to ../test-data/large-realistic-test-cases.json\n", len(scenarios))

	// Verify token counts for a sample
	fmt.Println("\nVerifying token counts for sample scenarios:")
	for _, sc := range scenarios[:5] {
		actualTokens := calculateTokenCount(sc.Prompt)
		difference := actualTokens - sc.InputTokens
		status := "✓"
		if abs(difference) > 200 { // Tolerate up to 200 token difference
			status = "⚠"
		}
		fmt.Printf("%s %s (%s): Target=%d, Actual=%d, Diff=%d\n",
			status, sc.ID, sc.Type, sc.InputTokens, actualTokens, difference)
	}
